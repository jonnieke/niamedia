    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <div className={`flex items-center gap-0.5 p-0.5 rounded-lg border text-xs font-bold ${
            ...
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-bold transition-all cursor-pointer ${
                currency === 'KES'
                  ? (dark ? 'bg-purple-600 text-white shadow-sm' : 'bg-white text-purple-700 shadow-sm')
                  : (dark ? 'text-white/60 hover:text-white' : 'text-gray-600 hover:text-gray-900')
              }`}
              title="Show prices in Kenyan Shillings (KES)"
            >
              KES
            </button>
            <button
              ...
              title="Show prices in US Dollars ($ USD)"
            >
              USD ($)
            </button>
          </div>
        ...
        >
          ...
          <div className={`mt-4 pt-4 border-t space-y-2 ${dark ? 'border-white/10' : 'border-gray-100'}`}>
            ...
            <div className="flex items-center justify-between py-1 px-1 mb-2">
              <span className={`text-xs font-semibold flex items-center gap-1.5 ${dark ? 'text-white/60' : 'text-gray-500'}`}>
                <Globe size={13} /> Display Currency
              </span>
              <div className={`flex items-center gap-1 p-0.5 rounded-lg border text-xs font-bold ${
                dark ? 'bg-white/5 border-white/10' : 'bg-gray-100 border-gray-200'
              }`}>
                <button
                  type="button"
