      ...
      {plan.popular && (
        <>
          ...
          <div className="absolute -top-3 left-1/2 -translate-x-1/2">
            <span className="px-3 py-1 rounded-full text-xs font-bold text-white whitespace-nowrap" style={{ background: 'linear-gradient(135deg, #8b5cf6, #3b82f6)' }}>Most Popular</span>
          </div>
        </>
      )}
...
function VideoCard({ plan }: { plan: typeof videoPlans[0] }) {
  return (
    <div className={`relative rounded-2xl p-7 flex flex-col border ${
      plan.popular ? 'border-purple-500/40' : 'border-white/8 bg-white/2'
    }`}
      style={plan.popular ? { background: 'linear-gradient(145deg, rgba(139,92,246,0.12), rgba(59,130,246,0.06))' } : {}}>
      {plan.popular && (
        <>
      ...
      <div className="flex items-center justify-between mt-2 mb-4">
        ...
        <span className="px-2 py-0.5 rounded-md text-[10px] font-bold border border-white/10 text-gray-400">{plan.tag}</span>
      </div>
      <p className="text-3xl font-extrabold text-white mb-0.5">KES {plan.price.toLocaleString()}</p>
...
function AudioRow({ plan }: { plan: typeof audioPlans[0] }) {
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/2 p-6 flex flex-col">
        ...
        {plan.prices.map((p, i) => (
          <button key={p.label} onClick={() => setSel(i)}
            className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all ${
              sel === i ? 'text-white border-purple-500/50 bg-purple-500/15' : 'border-white/8 text-gray-400 hover:border-white/20'
            }`}>{p.label}</button>
        ))}
      </div>
      ...
      <div className="pt-24 pb-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">One platform. Three services.</h1>
            <p className="text-base text-gray-400">Campaign copy, video production, and audio — each with its own pricing. Pay for what you need.</p>
          </div>

          {/* Service switcher */}
          <div className="flex justify-center mb-12">
            <div className="flex gap-2 p-1.5 rounded-2xl border border-white/8 bg-white/3">
              {([
          ...
          {service === 'audio' && (
            <>
              ...
              <div className="mt-6 p-5 rounded-2xl border border-white/8 bg-white/2 max-w-2xl mx-auto text-center">
                <p className="text-sm font-bold text-white mb-1">Need a full radio package?</p>
                <p className="text-xs text-gray-400 mb-4">3 × 30s spots for A/B campaign rotation — KES 14,000</p>
          ...
          <div className="mt-16 grid sm:grid-cols-3 gap-4 text-center">
            {[
              ...
              <div key={g.title} className="rounded-2xl border border-white/6 bg-white/2 p-5">
                <p className="text-sm font-bold text-white mb-2">{g.title}</p>
          ...
          <div className="mt-16 max-w-2xl mx-auto">
            ...
            <div className="space-y-2">
              {faqs.map((faq, i) => (
                <div key={i} className="rounded-xl border border-white/8 bg-white/2 overflow-hidden">
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                    className="w-full flex items-center justify-between px-5 py-4 text-left text-sm font-semibold text-white hover:text-purple-300 transition-colors">
                    {faq.q}
                    {openFaq === i ? <ChevronUp size={15} className="text-purple-400 shrink-0" /> : <ChevronDown size={15} className="text-gray-500 shrink-0" />}
                  </button>
                  {openFaq === i && (
                    <div className="px-5 pb-4 text-sm text-gray-400 leading-relaxed border-t border-white/6 pt-3">
                      {faq.a}
                    </div>
                  )}
                </div>
          ...
          {/* Bottom CTA */}
          <div className="mt-16 text-center p-10 rounded-3xl border border-white/8"
            style={{ background: 'linear-gradient(145deg, rgba(139,92,246,0.08), rgba(59,130,246,0.05))' }}>
            <h2 className="text-2xl font-bold text-white mb-2">Ready to create something great?</h2>
            <p className="text-gray-400 mb-6 text-sm">Start free with Concept Studio — spin your idea into a professional script. No card required.</p>
