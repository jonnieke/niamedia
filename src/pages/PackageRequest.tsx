      ...
      {options.map(o => (
        <button key={o} type="button" onClick={() => toggle(o)}
          className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
            active(o) ? 'border-purple-500/60 bg-purple-500/20 text-white' : 'border-white/10 text-gray-400 hover:border-white/20'
          }`}>{o}</button>
      ))}
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-7">
      ...
      <div className="card-glow p-5">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Your Details</p>
      ...
      {service === 'campaign' && (
        <div className="card-glow p-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Campaign Details</p>
      ...
      {service === 'video' && (
        <div className="card-glow p-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Video Brief</p>
          <div className="space-y-5">
            <div>
                ...
                {[{ v: true, l: 'Yes — I\'ll provide it' }, { v: false, l: 'No — write one for me' }].map(({ v, l }) => (
                  <button key={l} type="button" onClick={() => setVidHasScript(v)}
                    className={`flex-1 py-2.5 px-4 rounded-xl border text-sm font-medium transition-all ${
                      vidHasScript === v ? 'border-purple-500/50 bg-purple-500/10 text-white' : 'border-white/8 text-gray-400 hover:border-white/15'
                    }`}>{l}</button>
                ))}
              </div>
            </div>
      ...
      {service === 'audio' && (
        <div className="card-glow p-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Audio Brief</p>
          <div className="space-y-5">
            <div>
      ...
      {service === 'multi' && (
        <div className="card-glow p-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Multi-Service Brief</p>
          <div className="space-y-5">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(['Campaign Copy', 'Video Production', 'Audio Studio'] as const).map(s => (
                  <button key={s} type="button" onClick={() => setCampPlatforms(p => p.includes(s) ? p.filter(x => x !== s) : [...p, s])}
                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                      campPlatforms.includes(s) ? 'border-purple-500/60 bg-purple-500/20 text-white' : 'border-white/10 text-gray-400 hover:border-white/20'
                    }`}>{s}</button>
                ))}
              </div>
            </div>
      ...
      {/* Timeline + Budget */}
      <div className="card-glow p-5">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b border-white/6">Timeline & Budget</p>
  ...
  const content = (
    <>
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-white">Request a Package</h1>
