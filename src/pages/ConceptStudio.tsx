...
const FORMAT_OPTIONS: { id: ScriptFormat; label: string; icon: typeof Film; duration: string; desc: string }[] = [
  ...
  { id: 'documentary',   label: 'Mini Doco',      icon: BookOpen, duration: '3â€“5 min',    desc: 'Depth & authenticity.' },
  { id: 'short-film',    label: 'Short Film',     icon: Film,     duration: '5â€“10 min',   desc: 'Narrative & character.' },
]
...
const VOICE_DEMOS = [
  ...
  'New coastal resort opening in Watamu. Luxury but accessible. Target Nairobi middle class, age 28â€“45.',
...
function ScriptDisplay({ concept }: { concept: GeneratedConcept }) {
  const divider = 'â”€'.repeat(52)

  return (
    <div className="font-mono text-[13px] leading-relaxed text-gray-200 space-y-0">
      {/* Header */}
      <div className="text-center mb-4">
        <p className="text-purple-400 font-bold text-xs tracking-widest uppercase mb-1">Nia Media â€” Concept Brief</p>
      ...
      <div className="mb-4">
        ...
        <p className="text-gray-600 italic leading-snug">"{concept.logline}"</p>
      </div>
      ...
      <div className="mb-4">
        ...
        {concept.synopsis.split('\n\n').map((para, i) => (
          <p key={i} className="text-gray-600 mb-2 leading-snug">{para}</p>
        ))}
      </div>
          ...
          {concept.scenes.map((scene) => (
            <div key={scene.number}>
              <p className="text-white font-bold mb-1">
                SCENE {scene.number} â€” {scene.title.toUpperCase()}
                <span className="text-gray-500 font-normal ml-2">[{scene.timecode}]</span>
              </p>
              <div className="pl-3 border-l border-gray-200 space-y-1">
                <p><span className="text-blue-400">VISUAL  </span><span className="text-gray-600">{scene.visual}</span></p>
                <p><span className="text-amber-400">AUDIO   </span><span className="text-gray-600">{scene.audio}</span></p>
                <p><span className="text-emerald-400">VO      </span><span className="text-gray-200">{scene.vo}</span></p>
                {scene.onScreen && (
                  <p><span className="text-purple-400">SUPER   </span><span className="text-gray-600">{scene.onScreen}</span></p>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="mb-2">
        ...
        <p><span className="text-gray-500">STYLE     </span><span className="text-gray-600">{concept.productionNotes.recommendedStyle}</span></p>
        <p><span className="text-gray-500">MOOD      </span><span className="text-gray-600">{concept.productionNotes.mood}</span></p>
        <p><span className="text-gray-500">COLOUR    </span><span className="text-gray-600">{concept.productionNotes.colorDirection}</span></p>
        <p><span className="text-gray-500">MUSIC     </span><span className="text-gray-600">{concept.productionNotes.musicDirection}</span></p>
      </div>

      <p className="text-gray-600 text-[11px] mt-4">{divider}</p>
      <p className="text-center text-gray-600 text-[11px] mt-1">Generated by Nia Media AI â€” niamedia.co</p>
    </div>
  )
}
  ...
  const handleDownload = () => {
    ...
    const lines = [
      ...
      ...concept.scenes.flatMap(s => [
        '',
        `SCENE ${s.number} â€” ${s.title.toUpperCase()} [${s.timecode}]`,
        `  VISUAL  ${s.visual}`,
        `  AUDIO   ${s.audio}`,
        `  VO      ${s.vo}`,
      ...
      '',
      divider,
      'Generated by Nia Media AI â€” niamedia.co',
    ]
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl font-bold text-gray-900">Concept Studio</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="w-full lg:w-80 xl:w-96 shrink-0 space-y-4 relative z-10">
          ...
          <div className="card-glow p-5">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Your Idea</p>
              ...
              {(['text', 'voice', 'images'] as const).map(tab => (
                <button key={tab} onClick={() => setInputTab(tab)}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold capitalize transition-all ${
                    inputTab === tab ? 'text-white bg-purple-500/20 border-b-2 border-purple-500' : 'text-gray-500 hover:text-gray-600'
                  }`}>
            ...
            {inputTab === 'text' && (
              <textarea
                ...
                placeholder="Describe your idea, product, business, or story you want to tell... (e.g. 'A real estate company in Ruaka targeting young professionals â€” 2-bedroom apartments from KES 6.5M')"
                value={textInput}
              ...
              <div className="space-y-3">
                <button onClick={handleVoiceToggle}
                  className={`w-full py-6 rounded-xl border flex flex-col items-center gap-3 transition-all ${
                    isRecording
                      ...
                      : 'border-gray-200 bg-white/3 hover:border-purple-500/30'
                  }`}>
                  {isRecording ? (
                    <>
                    ...
                    <>
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-800">Tap to record your idea</p>
                        <p className="text-xs text-gray-500 mt-0.5">Speak naturally â€” describe your business and goal</p>
                      </div>
                    </>
                  )}
                </button>
                {voiceTranscript && (
                  <div className="rounded-xl p-4 border border-emerald-500/20 bg-emerald-500/5">
                    ...
                    <p className="text-sm text-gray-600 leading-relaxed">{voiceTranscript}</p>
            ...
            {inputTab === 'images' && (
              <div className="space-y-3">
                ...
                <button onClick={() => fileRef.current?.click()}
                  className="w-full py-6 rounded-xl border border-dashed border-gray-300 bg-white/2 hover:border-purple-500/40 hover:bg-purple-500/5 flex flex-col items-center gap-2 transition-all">
                  <ImagePlus size={22} className="text-gray-500" />
                  <div>
                    <p className="text-sm font-semibold text-gray-800">Upload reference images</p>
                    <p className="text-xs text-gray-500 mt-0.5">Mood board, product shots, inspiration â€” PNG / JPG</p>
                  </div>
                </button>
                {images.length > 0 && (
                ...
                {images.length > 0 && (
                  <div className="rounded-xl p-3 border border-gray-200 bg-white/3">
                    <p className="text-xs text-gray-500 font-semibold mb-1">Add context (optional)</p>
          ...
          {/* Format selector */}
          <div className="card-glow p-5">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Output Format</p>
                ...
                <button key={opt.id} onClick={() => setFormat(opt.id)}
                  className={`flex flex-col gap-1 px-3 py-2.5 rounded-xl border text-left transition-all ${
                    format === opt.id
                      ...
                      : 'border-gray-200 bg-white/2 hover:border-gray-300 hover:bg-gray-50'
                  }`}>
          ...
          <button onClick={handleGenerate} disabled={!canGenerate || loading}
            className="w-full py-3.5 rounded-xl text-sm font-bold text-gray-900 flex items-center justify-center gap-2.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="flex-1 min-w-0">
          {!concept && !loading && (
            <div className="card-glow p-10 text-center h-full flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-base font-bold text-gray-900 mb-2">Your concept will appear here</p>
              <p className="text-sm text-gray-500 max-w-xs leading-relaxed">Describe your idea on the left â€” text, voice, or images â€” and hit Spin Concept to generate a professional script in seconds.</p>
              <div className="mt-8 grid grid-cols-3 gap-3 w-full max-w-sm">
                {['Commercial', 'Documentary', 'Short Film'].map(t => (
                  <div key={t} className="px-3 py-2 rounded-xl border border-gray-200 text-xs text-gray-500 text-center">{t}</div>
                ))}
              </div>
            </div>
          )}
          ...
          {loading && (
            <div className="card-glow p-10 text-center flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-base font-bold text-gray-900 mb-2">Spinning your concept...</p>
            ...
            <div className="card-glow overflow-hidden">
              ...
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200"
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="text-xs font-bold text-gray-900">{concept.title}</p>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handleDownload}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-500 border border-gray-200 bg-white/3 hover:border-gray-300 transition-all">
                    <Download size={11} />
                    Download
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
          ...
          <div className="w-full max-w-lg card-glow p-7 relative">
            <button onClick={() => setShowPaywall(false)}
              className="absolute top-4 right-4 w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-500 hover:text-white transition-colors">
              <X size={15} />
            </button>
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-gray-900 mb-1">Push "{concept?.title}" to Development</h2>
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                <Link key={pkg.name}
                  ...
                  className="flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-purple-500/40 bg-white/3 hover:bg-purple-500/8 transition-all group">
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-bold text-gray-900">{pkg.name}</p>
                      {pkg.badge && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-gray-900"
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/3 border border-gray-200">
              <AlertCircle size={13} className="text-amber-400 shrink-0" />
              <p className="text-xs text-gray-500">Your concept is saved automatically. You can return and push it to development any time.</p>
            </div>
          </div>
        </div>
      )}
