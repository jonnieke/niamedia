    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg relative" onClick={e => e.stopPropagation()}>
        <button onClick={onClose}
          className="absolute -top-3 -right-3 z-10 w-8 h-8 rounded-full bg-white/10 border border-white/15 flex items-center justify-center hover:bg-white/20 transition-colors">
        ...
        <div className="rounded-2xl border border-white/12 overflow-hidden"
          ...
          <div className="p-8 text-center">
            ...
            <div className="rounded-xl border border-white/8 bg-white/3 p-5 text-left space-y-3 mb-6">
              {[
                ...
                <div key={label} className="flex items-center justify-between gap-4">
                  ...
                  <p className="text-xs font-semibold text-white text-right truncate">{value}</p>
                </div>
              ))}
            </div>

            <p className="text-xs text-gray-400 leading-relaxed mb-5 px-2">
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              {[
                ...
                <div key={label} className="rounded-lg border border-white/8 bg-white/2 py-2 px-3 text-center">
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
          ...
          <p className="text-gray-400 max-w-md mb-8">{msg.sub}</p>
  ...
  return (
    <DashboardLayout>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button onClick={() => navigate('/projects')} className="flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-6 transition-colors">
        ...
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2 py-0.5 rounded text-[10px] font-bold text-white/60 border border-white/10 uppercase tracking-wide">
            ...
            <p className="text-gray-400 text-sm mt-0.5">{project.package} &middot; Creator: {project.creator_name}</p>
          </div>
            ...
            {latestRevision && (
              <div className="rounded-xl border border-white/8 bg-white/2 p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs font-semibold text-white">Creator Notes — Iteration {latestRevision.iteration}</p>
                  <span className="text-xs text-gray-500 ml-auto">
                    {new Date(latestRevision.requested_at).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </span>
                </div>
                <p className="text-sm text-gray-300 leading-relaxed">{latestRevision.notes ?? '—'}</p>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {canReview ? (
              <div className="rounded-2xl border border-white/8 bg-white/3 p-4 space-y-3">
                <p className="text-sm font-semibold text-white mb-1">Your Decision</p>
                ...
                <button onClick={() => setAction(action === 'accept' ? null : 'accept')}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-sm font-medium transition-all ${
                    action === 'accept' ? 'border-green-500/50 bg-green-500/10 text-green-300' : 'border-white/8 text-gray-300 hover:border-green-500/30 hover:text-green-300'
                  }`}>
                ...
                <button onClick={() => setAction(action === 'revision' ? null : 'revision')}
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-sm font-medium transition-all disabled:opacity-40 ${
                    action === 'revision' ? 'border-blue-500/50 bg-blue-500/10 text-blue-300' : 'border-white/8 text-gray-300 hover:border-blue-500/30 hover:text-blue-300'
                  }`}>
                ...
                <button onClick={() => setAction(action === 'reject' ? null : 'reject')}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-sm font-medium transition-all ${
                    action === 'reject' ? 'border-red-500/50 bg-red-500/10 text-red-300' : 'border-white/8 text-gray-300 hover:border-red-500/30 hover:text-red-300'
                  }`}>
              ...
              <div className="rounded-2xl border border-white/8 bg-white/2 p-4 text-center text-sm text-gray-500">
