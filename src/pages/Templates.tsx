  ...
  return (
    <div className="card-glow p-6 hover:border-purple-500/30 transition-all duration-300 flex flex-col group">
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        {t.platforms.map(p => (
          <span key={p} className="px-2 py-0.5 bg-white/5 border border-white/8 text-gray-400 text-[11px] font-medium rounded-lg">{p}</span>
        ))}
      </div>
  ...
  return (
    <div className="card-glow p-6 hover:border-blue-500/30 transition-all duration-300 flex flex-col group">
      ...
      <div className="flex gap-2 mb-3">
        <span className="px-2 py-0.5 bg-white/5 border border-white/8 text-gray-400 text-[10px] font-semibold rounded-lg">{t.duration}</span>
        <span className="px-2 py-0.5 bg-white/5 border border-white/8 text-gray-400 text-[10px] font-semibold rounded-lg">{t.style}</span>
      </div>
      <p className="text-xs text-gray-500 leading-relaxed flex-1 mb-4">{t.description}</p>
...
function AudioCard({ t, search }: { t: typeof audioTemplates[0]; search: string }) {
  ...
  return (
    <div className="card-glow p-6 hover:border-green-500/30 transition-all duration-300 flex flex-col group">
      ...
      <div className="flex gap-2 mb-3">
        <span className="px-2 py-0.5 bg-white/5 border border-white/8 text-gray-400 text-[10px] font-semibold rounded-lg">{t.duration}</span>
        <span className="px-2 py-0.5 bg-white/5 border border-white/8 text-gray-400 text-[10px] font-semibold rounded-lg">{t.mood}</span>
      </div>
      <p className="text-xs text-gray-500 leading-relaxed flex-1 mb-4">{t.description}</p>
      ...
      <div className="mb-4 p-3 rounded-xl bg-white/3 border border-white/6">
        ...
        <p className="text-xs text-gray-400 leading-relaxed italic">"{t.prompt}"</p>
      </div>
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex gap-1 border-b border-white/6 w-full sm:w-auto">
          {tabs.map(t => {
            const Icon = t.icon
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2.5 text-xs font-semibold rounded-t-lg border-b-2 -mb-px transition-all ${
                  tab === t.id ? 'text-white border-current' : 'border-transparent text-gray-500 hover:text-gray-300'
                }`}
      ...
      <div className="pt-24 pb-20 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-xl mx-auto mb-14">
            <h1 className="text-4xl font-bold text-white mb-4">Creative Templates</h1>
