...
export default function PaymentCallback() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      // 1. Instant online quote deposit
      if (orderId && orderId.startsWith('quote_')) {
        const qId = orderId.replace('quote_', '')
        window.location.replace(`/quote?paid=true&quote_id=${qId}`)
        return
      }

      // 2. Video project milestone balance payment
      if (orderId && orderId.startsWith('proj_')) {
        const projectId = orderId.replace('proj_', '')
        const { data: proj } = await supabase.from('projects').select('token, balance_paid_at, business_name').eq('id', projectId).maybeSingle()
        if (proj?.token) {
          if (proj.balance_paid_at || attempts >= 4) {
            window.location.replace(`/delivery/${proj.token}?paid=true`)
            return
          }
          setOrderTitle(`Milestone balance — ${proj.business_name}`)
        }
      ...
      <div className="w-full max-w-md text-center">
        ...
        {status === 'loading' && (
          <div>
            ...
            <p className="text-gray-300 text-sm">This usually takes a few seconds. Please don't close this tab.</p>
          </div>
        )}
        ...
        {status === 'paid' && (
          <div>
            ...
            {type === 'invoice' ? (
              <>
                <p className="text-gray-300 text-sm mb-8">The invoice has been marked as paid. You can view it in your Invoices dashboard.</p>
                <Link to="/invoices"
              ...
              <>
                <p className="text-gray-300 text-sm mb-2">
                  Your order is now in the production queue. A Nia Media creative will begin work shortly.
                </p>
                <p className="text-gray-400 text-xs mb-8">You'll receive an email notification when your deliverable is ready for review.</p>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {import.meta.env.VITE_WHATSAPP_NUMBER && (
                <a href={`https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER}?text=Hi, I had a payment issue for my order.`}
        ...
        {status === 'timeout' && (
          <div>
            ...
            <p className="text-gray-400 text-xs mb-8">
              Check your <strong className="text-gray-200">Projects</strong> page in a few minutes, or contact support with your tracking ID:
